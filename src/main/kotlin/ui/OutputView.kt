...
class OutputView {
    ...
    fun printBenefitDetails(discount: List<Int>) {
        ...
        BenefitType.values().forEachIndexed { index, benefit ->
            if (discount[index] == 0) println(BENEFIT_DETAILS_NOTING.format(benefit.type))
            if (discount[index] != 0) println(BENEFIT_DETAILS.format(benefit.type, discount[index]))
        }
        println()
    }
    ...
    companion object {
        ...
        private const val EVENT_BENEFIT_PREVIEW = "\n12월 %d일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!"
        ...
        private const val BENEFIT_DETAILS_NOTING = "%s: 없음"
